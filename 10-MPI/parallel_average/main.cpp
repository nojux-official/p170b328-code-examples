...
const auto ELEMENTS_PER_PROCESS = 100;

double get_average(const double* data, int data_size);

double *generate_random_numbers(double *random_numbers, int total_elements);
...
int main() {
    ...
    if (rank == 0) {
        ...
        random_numbers = generate_random_numbers(random_numbers, total_elements);
    }
...
double *generate_random_numbers(double *random_numbers, const int total_elements) {
